...
@section('title', 'Manajemen Data Pengguna - SIM LAB STTAL Admin')
@section('header', 'Manajemen ' . ucfirst($role))
    ...
    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6">
        ...
        <div class="bg-white rounded-lg shadow-md border border-slate-200">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center p-6 border-b border-slate-200 gap-4">
                <h5 class="text-lg font-semibold text-slate-700">Daftar {{ ucfirst($role) }}</h5>

                <button class="btn-primary flex items-center gap-2 px-4 py-2 rounded-lg shadow-md transition-colors"
                    onclick="modalAction('{{ url($role . '/create') }}')">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            if (urlParams.get('open_modal') === 'true') {
                ...
                modalAction("{{ url('user/create') }}?role=" + role);

                if (role) {
                    window.history.replaceState(null, null, window.location.pathname + "?role=" + role);
                } else {
