<form action="{{ url('laboratorium/' . $laboratorium->id . '/update') }}" method="POST" id="form-edit" class="w-full validate">
    @csrf

    <div class="relative w-full max-w-2xl mx-auto bg-white rounded-lg shadow-lg">
        <div class="flex items-center justify-between px-6 py-4 border-b">
            <h5 class="text-lg font-semibold text-gray-800">Edit Laboratorium</h5>
        ...
        <div class="px-6 py-4 space-y-4">
            <div>
                ...
                <input type="text" name="nama_lab" id="nama_lab"
                    ...
                    required value="{{ $laboratorium->nama_lab }}">
            ...
            <div>
                ...
                <input type="number" name="kapasitas" id="kapasitas"
                    class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2
                           focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                    required value="{{ $laboratorium->kapasitas }}">
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3"
                    class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2
                           focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ $laboratorium->deskripsi }}</textarea>
            ...
            <div>
                ...
                <select name="status" id="status"
                    class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2
                           focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                    <option value="tersedia" {{ $laboratorium->status == 'tersedia' ? 'selected' : '' }}>Tersedia
                    </option>
                    <option value="maintenance" {{ $laboratorium->status == 'maintenance' ? 'selected' : '' }}>
                        Maintenance</option>
                    <option value="dipakai" {{ $laboratorium->status == 'dipakai' ? 'selected' : '' }}>Dipakai
                    </option>
                </select>
